...
const DataManagement = ({ isExpanded, onToggle }) => {
  ...
  const handleBackupFavorites = () => {
    ...
    link.href = url;
    link.download = `quran-favorites-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
  ...
  return (
    <div className="bg-surface rounded-lg border border-border overflow-hidden">
            ...
            <div className="space-y-3">
              <div className="p-3 bg-background rounded-lg border border-border">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <p className="text-lg font-data font-semibold text-primary">9</p>
                    <p className="text-xs text-text-secondary">Favorite Surahs</p>
                  </div>
                  <div>
                    <p className="text-lg font-data font-semibold text-primary">3</p>
                    <p className="text-xs text-text-secondary">Bookmarked Ayahs</p>
                  </div>
                  <div>
                    <p className="text-lg font-data font-semibold text-primary">1</p>
                    <p className="text-xs text-text-secondary">Reading Position</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-3 bg-background rounded-lg border border-border">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-text-secondary">
                    {offlineContent.downloadedSurahs} of {offlineContent.totalSurahs} Surahs
                  </p>
                </div>
              ...
              <div className="w-full bg-surface rounded-full h-2 mb-3">
                <div 
                  ...
                  style={{ width: `${(offlineContent.downloadedSurahs / offlineContent.totalSurahs) * 100}%` }}
                />
              </div>
            ...
            <div className="space-y-2">
              {storageItems.map((item, index) => (
                <div key={index} className="flex items-center justify-between p-2 bg-background rounded border border-border">
                  <div className="flex items-center space-x-3">
                    <Icon 
                      name={
                        ...
                        item.type === 'cache' ? 'HardDrive' :
                        item.type === 'user' ? 'User' : 'Settings'
                      } 
                      size={16} 
                      className="text-text-secondary" 
                    />
                  ...
                  <span className="text-sm font-data text-text-secondary">{item.size}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {showResetDialog && (
        <>
          <div className="fixed inset-0 bg-black bg-opacity-50 z-300" onClick={cancelReset} />
          <div className="fixed inset-4 lg:inset-auto lg:top-1/2 lg:left-1/2 lg:transform lg:-translate-x-1/2 lg:-translate-y-1/2 lg:w-96 z-400 bg-background rounded-lg shadow-gentle-lg">
            <div className="p-6">
              ...
              <p className="text-sm text-text-secondary mb-6">
                Are you sure you want to reset all settings to their default values? This will affect:
              </p>
