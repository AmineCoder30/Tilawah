...
const PrimaryTabNavigation = () => {
  ...
  const isActive = (path) => {
    ...
    if (path === '/surah-listing') {
      return location.pathname === '/surah-listing' || location.pathname === '/search-results';
    }
    if (path === '/surah-detail') {
      return location.pathname === '/surah-detail' || location.pathname === '/audio-player-interface';
    }
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center min-h-[48px] px-3 py-2 rounded-lg transition-all duration-200 ${
                  active
                    ? 'bg-primary-50 text-primary' :'text-text-secondary hover:text-text-primary hover:bg-surface-hover'
                }`}
